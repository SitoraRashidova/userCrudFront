...
const User = () => {
  ...
  const handleAddUser = () => {
    ...
    const emailExists = users.some((user) => user.email === userInfo.email);
    if (emailExists) {
      alert("Ustoz, ro'yxatdan o'tdingizku, axir");
      return;
    }

    // Check password length
    if (userInfo.password.length < 8) {
      alert("Diqqat, parol 8 tamas! Kamida 8 ta bo'lishi kerakda hay");
      return;
    }

    // Check if passwords match
    if (userInfo.password !== userInfo.confirmPassword) {
      alert("No, no, no nooo, Parollar mos emaaas!!!");
      return;
    }
  ...
  const handleUpdateUser = () => {
    ...
    alert("Your information has been updated successfully!"); // Display confirmation alert
  };
